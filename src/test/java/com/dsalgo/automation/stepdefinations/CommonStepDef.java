...
public class CommonStepDef {
    ...
    @Given("The user is successfully logged in DsAlgo page")
    public void the_user_is_successfully_logged_in_ds_algo_page() {
    	Map<String, String> specificRow = testData.get(0);
    	String url = specificRow.get("URL");
    	DriverFactory.getDriver().get(url);
    	queuePage.enterDetails();
    }
    
    @When("The user clicks on Get Strated link for Queue")
    public void the_user_clicks_on_get_strated_link_for_queue() {
    	queuePage.clickQueues();
    }
    
    @When("The user clicks on Get Strated link for Tree")
    public void the_user_clicks_on_get_strated_link_for_tree() {
    	treePage.clickTree();
    }
    
    @Given("the user is on the Home page")
    public void given_the_user_is_on_the_home_page() {
    	String pageTitle = queuePage.verifyTitleOfPage();
    	assertEquals(pageTitle, "NumpyNinja");
    }
    ...
    public void then_the_user_should_be_navigated_to_practice_questions_page() {
        ...
        assertEquals("Practice Questions",practiceTitle);
    ...
    public void then_the_user_should_be_navigated_to_the_codeexecutor_editor_page() {
        ...
        assertEquals(expectedTitle,codeEditorTitle);        
    }
